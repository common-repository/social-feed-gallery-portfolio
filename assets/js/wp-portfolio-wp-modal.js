...
(function( $, PortfolioWP ){
    ...
    var PortfolioWPModal = Backbone.Model.extend( {
        ...
        initialize: function( args ){
            ...
            var modalView = new PortfolioWP.modal['view']({
                'model': this,
            ...
            this.set( 'PortfolioWPModal', modalView );

        },

        open: function( $item ) {
            var wpMediaView = this.get( 'wpMediaView' ),
                PortfolioWPModal = this.get( 'PortfolioWPModal' );

            // Set the current item
            this.set( 'item', $item );
            // Render PortfolioWP View
            PortfolioWPModal.render();
            // Append PortfolioWPModalView to wpMediaView
            wpMediaView.content( PortfolioWPModal );
            // Open wpMediaView
            wpMediaView.open();
    ...
    var PortfolioWPModalView = Backbone.View.extend( {

        /**
        * The Tag Name and Tag's Class(es)
        */
        tagName:    'div',
        className:  'edit-attachment-frame mode-select hide-menu hide-router portfolio-wp-edit-popup',

        /**
        * Template
        * - The template to load inside the above tagName element
        */
        template:   wp.template( 'portfolio-wp-image-editor' ),
        ...
        */
        events: {
            ...
            'click .actions a.portfolio-wp-gallery-meta-submit':       'saveItem',
            'click .actions a.portfolio-wp-gallery-meta-submit-close': 'saveItemAndClose',
        ...
        * Initialize
        *
        * @param object model   PortfolioWPImage Backbone Model
        */
        initialize: function( args ) {
        ...
        */
        render: function() {
            ...
            // Get current Index.
            this.attachment_index = PortfolioWP.Items.indexOf( this.item );
            ...
            if ( this.attachment_index == ( PortfolioWP.Items.length - 1 ) ) {
        ...
        * wp.media.view.PortfolioWPGalleryError
        */
        renderError: function( error ) {
            ...
            // Define view
            var view = new wp.media.view.PortfolioWPGalleryError( {
                model: model
            } );
        ...
        */
        loadPreviousItem: function() {
            ...
            item = PortfolioWP.Items.at( this.attachment_index );
...
//console.log(item.get('id'));
            wp.PortfolioWP.Save.saveImage( item.get( 'id' ), function(){
        ...
        saveItemAndClose: function ( event ){
            ...
            clearInterval( wp.PortfolioWP.Save.updateInterval );
            wp.PortfolioWP.Save.saveImages( function(){
                // Get item view and render it.
    ...
    PortfolioWP.modal = {
        'model' : PortfolioWPModal,
        'view' : PortfolioWPModalView
    };
